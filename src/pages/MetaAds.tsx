...
export default function MetaAdsPage() {
  ...
  // Meta Ads is a Premium feature — show soft gate for free/starter
          ...
          <h1 className="text-2xl font-semibold text-foreground flex items-center gap-2">
            ...
            {!isUnlocked && (
              <Badge variant="outline" className="text-[10px] gap-1 font-normal ml-1">
                <Clock className="h-2.5 w-2.5" /> Premium Feature
              </Badge>
            )}
          </h1>
        ...
        <Card className="border-2 border-dashed border-primary/30 bg-gradient-to-br from-primary/5 to-primary/10">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground mb-2">
              Unlock Meta Ads Intelligence
            </h2>
            <p className="text-sm text-muted-foreground max-w-md mx-auto mb-2">
              Track competitor ad creative, messaging strategies, and spend patterns across Facebook and Instagram.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-3 text-xs text-muted-foreground mb-6">
            ...
            <p className="text-[10px] text-muted-foreground mt-3">
              Available on Premium plan · $99/month
            </p>
          </CardContent>
        </Card>
      )}

      {/* Preview of what they'll get — show demo ads with overlay */}
      {!isUnlocked && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-medium text-foreground">Preview: What you'll see</h3>
            <Badge variant="outline" className="text-[9px]">Sample Data</Badge>
          </div>
          <div className="relative">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 opacity-50 pointer-events-none select-none blur-[1px]">
              {DEMO_META_ADS.slice(0, 3).map((ad) => (
                <MetaAdCard key={ad.id} ad={ad as any} isDemo />
              ))}
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-card/90 backdrop-blur-sm rounded-xl border shadow-lg p-5 text-center">
                <Lock className="h-5 w-5 text-primary mx-auto mb-2" />
                <p className="text-sm font-medium text-foreground">Premium feature</p>
                <p className="text-xs text-muted-foreground mt-0.5 mb-3">Upgrade to access real ad intelligence</p>
      ...
      {isUnlocked && (
        <>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              <SelectTrigger className="w-44 h-8 text-sm"><SelectValue placeholder="All competitors" /></SelectTrigger>
              <SelectContent>
            ...
            <div className="relative flex-1 max-w-xs">
              ...
              <Input placeholder="Search ad copy…" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="pl-8 h-8 text-sm" />
            </div>
            {(filters.competitorId || filters.isActive !== undefined || filters.search) && (
              <Button variant="ghost" size="sm" className="h-8 text-xs gap-1" onClick={() => { setFilters({}); setSearchInput(""); }}>
          ...
          {loading ? (
            ...
            <Card className="border">
              <CardContent className="p-12 text-center">
                ...
                <Button onClick={handleFetch} disabled={fetching} className="gap-2">
                  <Download className="h-4 w-4" /> Fetch Competitor Ads
                </Button>
              </CardContent>
            </Card>
          ) : (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {ads.map((ad) => (
                  <MetaAdCard key={ad.id} ad={ad} onAnalyze={handleAnalyze} onSelect={setSelectedAd} analyzing={analyzingId === ad.id} />
                ))}
              </div>
              {totalPages > 1 && (
              ...
              {selectedAdData && (
                <div className="space-y-4 mt-4">
                  ...
                  {selectedAdData.ad_snapshot_url && (
                    <>
                      <Separator />
                      <a href={selectedAdData.ad_snapshot_url} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
                        <ExternalLink className="h-3.5 w-3.5" /> View on Meta Ad Library
                      </a>
                    </>
                  )}
                  <Separator />
                  {analysis ? (
                    ...
                    <Button onClick={() => handleAnalyze(selectedAdData.id)} disabled={analyzingId === selectedAdData.id} className="w-full gap-2">
                      {analyzingId === selectedAdData.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
